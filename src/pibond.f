     ...
     &,XSIJ,SSUMK,CONK,SDALIK,QI,SS,IG
...
C 
      ...
      pij_min =-0.39d0
                ...
                NTCH = NH + NC
                IF(KI.EQ.icarb.and.KJ.EQ.icarb) THEN !1
                      IF( (ABS(FLOAT(NH)-XNI(2)).GT.1.0d-8).OR.
     &                    (ABS( FLOAT(NC+NOX)-XNI(icarb)-XNI(ioxygen))
     &                    .GT.1.0d-8)) THEN 
                           CALL BCUINT(KI,KJ,XNI(2),XNI(1)+XNI(3)-1.0d0
     &                                   ,NH,NC+NOX - 1,
...
C Add oxygen bicubic spline
C  -travisk 
                ELSEIF (KI.EQ.icarb.and.KJ.EQ.ihyd) THEN 
                  IF(XNI(ioxygen).LE.pno_min ) THEN
                    IF ( (ABS(FLOAT(NH) - XNI(ihyd)).gt.1.0d-8) .OR.
     &                  (ABS(FLOAT(NC) - XNI(icarb)).gt.1.0d-8) )THEN
...
c$$$             WRITE(*,*)'Poop1 here'
c$$$             WRITE(*,*) 'Atom',I,EXNIJ
c$$$             WRITE(*,*) KI,KJ,XNI(icarb)+XNI(ihyd)-1.0d0
c$$$             WRITE(*,*) XNI(ioxygen),NTCH-1,NOX
...
c$$$                DEXNJ(:)=0.0d0
c$$$                IF(KJ.EQ.1) THEN
c$$$                      NH=INT(XNJ(2)+1.0D-12)
c$$$                      NC=INT(XNJ(1)+1.0D-12)
c$$$                      IF((ABS(FLOAT(NH)-XNJ(2)).GT.1.0d-8).OR.
c$$$     &                   (ABS(FLOAT(NC)-XNJ(1)).GT.1.0d-8)) THEN
c$$$                           CALL BCUINT(KJ,KI,XNJ(2),XNJ(1),NH,NC,
c$$$     &                                   EXNJI,DEXNJ(2),DEXNJ(1))
c$$$                      ELSE
c$$$                           EXNJI=XH(KI,NH,NC)
c$$$                           DEXNJ(2)=XH1(KI,NH,NC)
c$$$                           DEXNJ(1)=XH2(KI,NH,NC)
c$$$                      ENDIF
c$$$                          DEXNJ(ioxygen)=DEXNJ(icarb)
c$$$                ENDIF
C
C  Pji(I,NC,NH,NO)
C  -travisk
...
C  -travis
                ...
                NTCH = NH + NC
                IF(KJ.EQ.icarb.and.KI.EQ.icarb) THEN
...
C Add oxygen bicubic spline
C  -travisk 
                ELSEIF (KJ.EQ.icarb.and.KI.EQ.ihyd) THEN 
                  IF(XNJ(ioxygen).LE.pno_min ) THEN

                    IF ( (ABS(FLOAT(NH) - XNJ(ihyd)).gt.1.0d-8) .OR.
     &                  (ABS(FLOAT(NC) - XNJ(icarb)).gt.1.0d-8) )THEN
