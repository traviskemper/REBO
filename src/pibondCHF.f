...
C              WRITE(297,122) i,NA(i),XHC(i,1),XHC(i,2),XHC(i,iflor)
C                ENDIF
C                IF (mynode.EQ.1) THEN
C              WRITE(298,122) i,NA(i),XHC(i,1),XHC(i,2),XHC(i,iflor)
C                ENDIF
C122             FORMAT (2(1x,I4),iflor(F12.8))
C
500   CONTINUE
C
...
C
                ...
                XNI(1)=XHC(I,1)
                XNI(2)=XHC(I,2)
                XNI(iflor)=XHC(I,iflor)
                XNI(KJ)=XNI(KJ)-WW(J)
                QI=XNI(1)+XNI(2)+XNI(iflor)-3.0D0
                SDALIK=0.0D0
...
46                     CONTINUE
                     ...
                     DCFUNI(NK)=0.0d0
                     IF(KK.EQ.1) THEN
                          XX=XHC(KN,1)+XHC(KN,2)+XHC(KN,iflor)-FC-3.0d0
                          IF(XX.LT.3.0d0) THEN
                               IF(XX.LE.2.0d0) THEN
                                    CFUNI(NK)=1.0d0
                               ELSE
...
!            WRITE(*,*) FC,GANGLE,EXX
!            WRITE(*,*) KI,KJ,KK
!            WRITE(*,*) XDB(KI,KJ,KK),(SIJ-S3),REG(KI,KJ,KK)
! end debug           
C
                     DCTDJK=-2.0d0/SS
...
C
                ...
                XNJ(1)=XHC(JN,1)
                XNJ(2)=XHC(JN,2)
                XNJ(iflor)=XHC(JN,iflor)
                XNJ(KI)=XNJ(KI)-WW(J)
                QJ=XNJ(1)+XNJ(2)+XNJ(iflor)-3.0D0
                SDALJL=0.0D0
...
c                     IF((KL+KIKJ).EQ.3) THEN
                     IF(KL.EQ.1) THEN
                          XX=XHC(LN,1)+XHC(LN,2)+XHC(LN,iflor)-FC-3.0d0
                          IF(XX.LT.3.0d0) THEN
                               IF(XX.LE.2.0d0) THEN
                                    CFUNJ(NL)=1.0d0
                               ELSE
...
C
     ...
     &                   (ABS(FLOAT(NF)-XNI(iflor)).GT.1.0D-08).OR.
     &                   (ABS(FLOAT(NC)-XNI(1)).GT.1.0d-08)) THEN
C                           write(6,*) nf
                           CALL TCUINT(KI,KJ,XNI(2),XNI(1)
     &                    ,XNI(iflor),NH,NC,NF,EXNIJ,DEXNI(2),DEXNI(1),
     &                    DEXNI(iflor))
                      ELSE
                           EXNIJ=XHF(KJ,NH,NC,NF)
                           DEXNI(2)=XHF1(KJ,NH,NC,NF)
                           DEXNI(1)=XHF2(KJ,NH,NC,NF)
                           DEXNI(iflor)=XHF3(KJ,NH,NC,NF)
                      ENDIF
                ENDIF
C
                EXNJI=0.0d0
     ...
     &                   (ABS(FLOAT(NF)-XNJ(iflor)).GT.1.0D-08).OR.
     &                   (ABS(FLOAT(NC)-XNJ(1)).GT.1.0d-8)) THEN
                           CALL TCUINT(KJ,KI,XNJ(2),XNJ(1)
     &                     ,XNJ(iflor),NH,NC,NF,EXNJI,DEXNJ(2),DEXNJ(1)
     &                     ,DEXNJ(iflor))
                      ELSE
                           EXNJI=XHF(KI,NH,NC,NF)
                           DEXNJ(2)=XHF1(KI,NH,NC,NF)
                           DEXNJ(1)=XHF2(KI,NH,NC,NF)
                           DEXNJ(iflor)=XHF3(KI,NH,NC,NF)
                      ENDIF
C           IF (mynode.EQ.0) THEN
...
C
                ...
                XNT1=XNI(1)+XNI(2)+XNI(iflor)-2.0d0
                XNT2=XNJ(1)+XNJ(2)+XNJ(iflor)-2.0d0
