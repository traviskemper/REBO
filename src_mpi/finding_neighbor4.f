...
CCC 1-D parallel
      IF (NNDP.EQ.1) THEN
        ...
        if(3*RLIMAX.le.RLJMAX) then
	 node_cell_min_temp(NPD_i)=node_cell_min(NPD_i)+2
         node_cell_max_temp(NPD_i)=node_cell_min(NPD_i)+4
        else
         node_cell_min_temp(NPD_i)=node_cell_min(NPD_i)+4
         node_cell_max_temp(NPD_i)=node_cell_min(NPD_i)+4
        endif
C Find the left side atoms that will be passed to left node
     ...
     &        *nncell(1)
              ...
              DO while (ii.NE.0)
                ...
                IF ((R_i.GT.3*RLIMAX).AND.(R_i.LE.RLJMAX)) THEN
C these atoms are moving to left node
        ...
        node_cell_max_temp=node_cell_max
        if(3*RLIMAX.le.RLJMAX) then
         node_cell_min_temp(NPD_i)=node_cell_max(NPD_i)-4
         node_cell_max_temp(NPD_i)=node_cell_max(NPD_i)-2
        else
         node_cell_min_temp(NPD_i)=node_cell_max(NPD_i)-4
         node_cell_max_temp(NPD_i)=node_cell_max(NPD_i)-4
        endif
...
C these atoms are moving to RIGHT node
          ...
          ENDDO
        ENDDO
        IF (NLL.GT.15*NBMAX1) THEN
          WRITE(*,*) NLL,' NLL is GT 15*NBMAX1',15*NBMAX1,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NRR.GT.15*NBMAX1) THEN
          WRITE(*,*) NRR,' NRR is GT 15*NBMAX1',15*NBMAX1,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        N_atom_passLL(3)=NLL/15
        N_atom_passRR(3)=NRR/15
c        WRITE(*,*) 'find_nabor4,NLL,NRR,',mynode,NLL/15,NRR/15
CCC 2-D parallel
      ELSE
        node_cell_min_temp=node_cell_min
        node_cell_max_temp=node_cell_max
...
C these atoms are moving to RIGHT node
        ...
        node_cell_max_temp=node_cell_max
        if(3*RLIMAX.le.RLJMAX) then
         node_cell_min_temp(NPD_j)=node_cell_min(NPD_j)+2
         node_cell_max_temp(NPD_j)=node_cell_min(NPD_j)+4
        else
         node_cell_min_temp(NPD_j)=node_cell_min(NPD_j)+4
         node_cell_max_temp(NPD_j)=node_cell_min(NPD_j)+4
        endif
C Find the DOWN side atoms that will be passed to DOWN node
     ...
     &        *nncell(1)
              ...
              DO while (ii.NE.0)
                ...
                IF ((R_j.GT.3*RLIMAX).AND.(R_j.LE.RLJMAX)) THEN
C these atoms are moving to DOWN node
        ...
        node_cell_max_temp=node_cell_max
        if(3*RLIMAX.le.RLJMAX) then
         node_cell_min_temp(NPD_j)=node_cell_max(NPD_j)-4
         node_cell_max_temp(NPD_j)=node_cell_max(NPD_j)-2
        else
         node_cell_min_temp(NPD_j)=node_cell_max(NPD_j)-4
         node_cell_max_temp(NPD_j)=node_cell_max(NPD_j)-4
        endif
...
C Find the LEFT DOWN side atoms that will be passed to LEFT DOWN side
     ...
     &        *nncell(1)
              ...
              DO while (ii.NE.0)
                R_i=ABS(R0_node(NPD_i,ii)-pmin_node(NPD_i))
                R_j=ABS(R0_node(NPD_j,ii)-pmin_node(NPD_j))
                IF (((R_i.GT.3*RLIMAX).AND.(R_i.LE.RLJMAX).AND.
     &             (R_j.LE.RLJMAX)).OR.((R_j.GT.3*RLIMAX).AND.
     &             (R_j.LE.RLJMAX).AND.(R_i.LE.RLJMAX))) THEN
...
C these atoms are moving to RIGHT UP node
     ...
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NDD.GT.15*NBMAX2) THEN
          WRITE(*,*) NDD,' NDD is GT 15*NBMAX2',15*NBMAX2,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NUU.GT.15*NBMAX2) THEN
          WRITE(*,*) NUU,' NUU is GT 15*NBMAX2',15*NBMAX2,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NLD.GT.15*NBMAX3) THEN
          WRITE(*,*) NLD,'NLD is GT 15*NBMAX3',15*NBMAX3,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NLU.GT.15*NBMAX3) THEN
          WRITE(*,*) NLU,'NLU is GT 15*NBMAX3',15*NBMAX3,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NRD.GT.15*NBMAX3) THEN
          WRITE(*,*) NRD,'NRD is GT 15*NBMAX3',15*NBMAX3,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        IF (NRU.GT.15*NBMAX3) THEN
          WRITE(*,*) NRU,'NRU is GT 15*NBMAX3',15*NBMAX3,
     &    'increase Density in allocate_arrays.f fd_nabor4'
          STOP
        ENDIF
        N_atom_passLL(3)=NLL/15
...
C        WRITE(*,*)'find_nabor4',mynode,NLL/15,NRR/15,NDD/15,NUU/15,'LL'
C        WRITE(*,*)'find_nabor4',mynode,NLD/15,NLU/15,NRD/15,NRU/15,'LD'
      ENDIF
