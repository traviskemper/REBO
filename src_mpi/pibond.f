...
C  Flourine is still left in at this point however it is not considered
C  for this initial CHO parameterization
C -travisk
                NTCH = NH + NC
                IF(KI.EQ.icarb.and.KJ.EQ.icarb) THEN !1
                      IF((ABS(FLOAT(NH)-XNI(ihyd)).GT.1.0d-08).OR.
     &                   (ABS(FLOAT(NF)-XNI(iflor)).GT.1.0D-08).OR.
     &    (ABS(FLOAT(NC+NOX)-XNI(icarb)-XNI(ioxygen)).GT.1.0d-08)) THEN
C                           write(6,*) nf
                           CALL TCUINT(I,JN,KI,KJ,XNI(2),XNI(1),XNI(3)
     &                    ,NH,NC+NOX-1,NF,EXNIJ,DEXNI(2),DEXNI(1),
     &                    DEXNI(3))
                      ELSE
     ...
     &                    DEXNI(3))
                  ...
                  ELSEIF ( XNI(ioxygen).GE.pno_max) THEN 
                    IF((ABS(FLOAT(NTCH)-XNI(icarb)-XNI(ihyd)-XNI(iflor)
     &                                               ).GT.1.0d-8)
     &             .or.(ABS(FLOAT(NOX) - XNI(ioxygen)).GT.1.0d-8)) THEN
                       CALL OXYBCUINT (KI,KJ
     &                           ,XNI(icarb)+XNI(ihyd)+XNI(iflor)-2.0d0
     &                           ,XNI(ioxygen),NTCH-1,NOX,EXNIJ
     &                           ,DEXNI(icarb),DEXNI(ioxygen) )
                        DEXNI(ihyd) = DEXNI(icarb)
                        DEXNI(iflor) = DEXNI(icarb)
                      IF ( EXNIJ.LT.pij_min) THEN
                         EXNIJ = pij_min
                      ENDIF
                    ELSE
                      EXNIJ  = XHO(KI,KJ,NTCH-2,NOX)
                      DEXNI(icarb) = XHO1(KI,KJ,NTCH-2,NOX)
                      DEXNI(ioxygen) = XHO2(KI,KJ,NTCH-2,NOX)
                      DEXNI(ihyd) = DEXNI(icarb) 
                      DEXNI(iflor) = DEXNI(iflor) 
                    ENDIF
