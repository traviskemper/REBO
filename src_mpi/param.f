...
c
     ...
     &,xnlow,xnhg,ynlow,ynhg,znlow,znhg,dx1,dx2,dy1,dy2,dz1,dz2,dz3,dz4

C local variable
      REAL*8 CLM_value(4,16),CLMN_value(64),TLMN_value(64)
C
C Angular function spline values
C   should be calculated with qucof x
...
C
C read integer values of tricubic spline
...
119      CONTINUE
      IF (mynode.EQ.0) THEN
         READ(15,*) I,J,K,l,XHH
      ENDIF
C      WRITE(*,*) 'finish read 1st part 15 head'
      ...
      CALL MPI_BCAST(XHH,1,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
...
C
C
c$$$      XH1(2,3,1,1)=(XH(2,4,1,1)-XH(2,2,1,1))/2.0D0
c$$$      XH1(2,2,2,1)=(XH(2,3,2,1)-XH(2,1,2,1))/2.0D0
c$$$C
c$$$      XH2(2,2,2,1)=(XH(2,2,3,1)-XH(2,2,1,1))/2.0D0
c$$$      XH2(2,1,3,1)=(XH(2,1,4,1)-XH(2,1,2,1))/2.0D0
c$$$cccc
c$$$      XH3(iflor,1,1,3)=(XH(iflor,1,1,4)-XH(iflor,1,1,2))/2.0D0
c$$$      XH3(iflor,1,2,2)=(XH(iflor,1,2,3)-XH(iflor,1,2,1))/2.0D0
c$$$C
c$$$      XH2(iflor,1,2,2)=(XH(iflor,1,3,2)-XH(iflor,1,1,2))/2.0D0
c$$$      XH2(iflor,1,3,1)=(XH(iflor,1,4,1)-XH(iflor,1,2,1))/2.0D0
c$$$
c$$$      XH3(2,1,1,3)=(XH(2,1,1,4)-XH(2,1,1,2))/2.0D0
c$$$      XH1(iflor,3,1,1)=(XH(iflor,4,1,1)-XH(iflor,2,1,1))/2.0D0
c$$$
c$$$      XH3(2,1,2,2)=(XH(2,1,2,3)-XH(2,1,2,1))/2.0D0
c$$$      XH1(iflor,2,2,1)=(XH(iflor,3,2,1)-XH(iflor,1,2,1))/2.0D0
c$$$
c$$$      XH2(2,1,2,2)=(XH(2,1,3,2)-XH(2,1,1,2))/2.0D0
c$$$      XH2(iflor,2,2,1)=(XH(iflor,2,3,1)-XH(iflor,2,1,1))/2.0D0

      XH1(2,3,1,1)=(XH(2,4,1,1)-XH(2,2,1,1))/2.0D0
      XH1(2,2,2,1)=(XH(2,3,2,1)-XH(2,1,2,1))/2.0D0
C
      XH2(2,2,2,1)=(XH(2,2,3,1)-XH(2,2,1,1))/2.0D0
      XH2(2,1,3,1)=(XH(2,1,4,1)-XH(2,1,2,1))/2.0D0
cccc
      XH3(3,1,1,3)=(XH(3,1,1,4)-XH(3,1,1,2))/2.0D0
      XH3(3,1,2,2)=(XH(3,1,2,3)-XH(3,1,2,1))/2.0D0
C
      XH2(3,1,2,2)=(XH(3,1,3,2)-XH(3,1,1,2))/2.0D0
      XH2(3,1,3,1)=(XH(3,1,4,1)-XH(3,1,2,1))/2.0D0

      XH3(2,1,1,3)=(XH(2,1,1,4)-XH(2,1,1,2))/2.0D0
      XH1(3,3,1,1)=(XH(3,4,1,1)-XH(3,2,1,1))/2.0D0

      XH3(2,1,2,2)=(XH(2,1,2,3)-XH(2,1,2,1))/2.0D0
      XH1(3,2,2,1)=(XH(3,3,2,1)-XH(3,1,2,1))/2.0D0

      XH2(2,1,2,2)=(XH(2,1,3,2)-XH(2,1,1,2))/2.0D0
      XH2(3,2,2,1)=(XH(3,2,3,1)-XH(3,2,1,1))/2.0D0

C      WRITE(*,*) 'finish read 1st part 15'
C
...
C
      DO nn=1,3*6*6*6
        IF (mynode.EQ.0) THEN
           READ(15,*) I,L,M,N
C           WRITE(*,*) nn,i,l,m,n,'start read 15 part 2'
           DO j=1,16
             READ(15,*) CLM_value(1,j),CLM_value(2,j),
     &       CLM_value(3,j),CLM_value(4,j)
        ...
        CALL MPI_BCAST(CLM_value,64,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
        kk=0
        DO ii=1,16
          DO jj=1,4
            kk=kk+1
            CLM(I,L,M,N,kk)=CLM_value(jj,ii)
          ENDDO
        ENDDO
      ENDDO
C127   CONTINUE
C      WRITE(*,*) CLM(3,3,4,1,10),'finish read 15'
C
      IC=0
C
      DO 9 I=1,4
...
C
      IF (mynode.EQ.0) THEN
         READ(14,*) I3D
      ENDIF
      CALL MPI_BCAST(I3D,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
C  129 CONTINUE
      DO nn=1,4*4*9
        IF (mynode.EQ.0) THEN
          READ(14,*) L,M,N
        ...
        CALL MPI_BCAST(N,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        IF (mynode.EQ.0) THEN
          READ(14,*) (CLMN_value(I),I=1,64)
        ENDIF
        CALL MPI_BCAST(CLMN_value,64,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        DO i=1,64
          CLMN(1,L,M,N,I)=CLMN_value(I)
        ENDDO
      ENDDO
C      WRITE(*,*) CLMN(1,1,1,2,20),'finish read 14'
...
C  229 CONTINUE
      DO nn=1,6*6*3
        ...
        CALL MPI_BCAST(N,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        IF (mynode.EQ.0) THEN
           READ(17,*) (CLMN_value(I),I=1,64)
        ENDIF
        CALL MPI_BCAST(CLMN_value,64,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        DO i=1,64
          CLMN(3,L,M,N,I)=CLMN_value(I)
        ENDDO
      ENDDO
C      WRITE(*,*) CLMN(3,1,1,2,20),'finish read 17'
C      GO TO 229
C  230 CONTINUE
C
      IF (mynode.EQ.0) THEN
...
C  239 CONTINUE
      DO nn=1,4*4*9
        ...
        CALL MPI_BCAST(N,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        IF (mynode.EQ.0) THEN
           READ(18,*) (CLMN_value(I),I=1,64)
        ENDIF
        CALL MPI_BCAST(CLMN_value,64,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        DO i=1,64
          CLMN(2,L,M,N,I)=CLMN_value(I)
        ENDDO
      ENDDO
C      WRITE(*,*) CLMN(2,1,1,2,20),'finish read 18'
C      GO TO 239
C  240 CONTINUE
C
      DO 134 L=1,10
...
C
      IF (mynode.EQ.0) THEN
         READ(16,*) ITD
      ENDIF
      CALL MPI_BCAST(ITD,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
C  135 CONTINUE
      DO nn=1,6*6*3
        IF (mynode.EQ.0) THEN
           READ(16,*) L,M,N
        ...
        CALL MPI_BCAST(N,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
        IF (mynode.EQ.0) THEN
           READ(16,*) (TLMN_value(I),I=1,64)
        ENDIF
        CALL MPI_BCAST(TLMN_value,64,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        DO i=1,64
          TLMN(L,M,N,I)=TLMN_value(I)
        ENDDO
      ENDDO
C      WRITE(*,*) TLMN(1,1,2,20),'finish read 16'
C      GO TO 135
C  136 CONTINUE
C
C      WRITE(*,*) 'finish read spline files'
      DO 140 L=1,10
           DO 139 M=1,10
                DO 138 N=4,10
                     DO 137 I=1,64
...
C Overwrite previous spline values with refetted to dissociation energies
       XHO( 1 , 1 , 1 , 2 )= 2.6322758289717174E-001   !  
       XHO( 1 , 1 , 1 , 3 )= 1.8172173808666534E-001   !  
       XHO( 1 , 1 , 1 , 4 )= -3.3812822082183042E-001   !  
       XHO( 1 , 1 , 2 , 2 )= 6.8975879892882921E-003   !  
       XHO( 1 , 1 , 2 , 3 )= -1.4590703313216463E-001   !  
       XHO( 1 , 1 , 3 , 2 )= -1.8833795634238631E-001   !  
       XHO( 1 , 2 , 1 , 2 )= 3.3634981543859399E-001   !  
       XHO( 1 , 2 , 1 , 3 )= 6.3516806142203908E-001   !  
       XHO( 1 , 2 , 1 , 4 )= -5.0686982615829446E-001   !  
       XHO( 1 , 2 , 2 , 2 )= 1.9144348570035366E-001   !  
       XHO( 1 , 2 , 2 , 3 )= -1.4434919479893391E-001   !  
       XHO( 1 , 2 , 3 , 2 )= -3.5365613785081512E-001   !  
...
! end debug
         ...
         ENDDO   
      ENDDO  
C Sulfr angular parameters
C S8 - anglular
!      a_s0 = 9.764333800000001d-005
!      a_s1 = 1.367443000000000d-001
!      a_s2 = -3.148670700000000d-001
       SPGS(isulfur, 1 )= 2.0729912836725907E-002 
       SPGS(isulfur, 2 )= 6.8841620886410479E-002 
       SPGS(isulfur, 3 )= 8.0539453257486682E-002 
       SPGS(isulfur, 4 )= -7.7424514264693531E-002 
       SPGS(isulfur, 5 )= -5.3677335857774983E-002 
       SPGS(isulfur, 6 )= 2.6181908430954994E-002 
       SPGS(isulfur, 7 )= 1.4131093540527410E-002 
...
! weekend X-S-C angle for thiirane  
...
C Hydrogen
C
C Need to change to new values for H2
C   -travisk
c$$$      b1(ihyd,ihyd)    = 28.2297
c$$$      beta1(ihyd,ihyd) =  1.708
c$$$      b2(ihyd,ihyd)    =  0.d0
c$$$      beta2(ihyd,ihyd) =  1.d0
c$$$      b3(ihyd,ihyd)    =  0.d0
c$$$      beta3(ihyd,ihyd) =  1.d0
c$$$      capa(ihyd,ihyd)  = 31.6731
c$$$      alfa(ihyd,ihyd)  =  3.536
c$$$      capq(ihyd,ihyd)  =  0.370
...
C    Pair potential values for Sulfur
C   SULFUR-SULFUR
...
C Cut off regions
C O-O
C      dijmin(ioxygen,ioxygen) = 1.55d0
C      dijmax(ioxygen,ioxygen) = 1.7d0
...
C C-O
C       dijmin(icarb,ioxygen) = 1.6d0
C       dijmax(icarb,ioxygen) = 1.9d0
...
C O-H
C      dijmin(ihyd,ioxygen) = 1.3d0
C      dijmax(ihyd,ioxygen) = 1.7d0
      ...
      DO I = 1, RTYPES
        DO J = 1,RTYPES
         ...
         ELSEIF  (RB1(I,J).GT.0.0d0) THEN
           RB1(J,I) = RB1(I,J)
         ELSE
           IF(mynode.EQ.0)WRITE(13,*) 'RB1 not set for',I,J
         ENDIF
         ...
         ELSEIF  (RB2(I,J).GT.0.0d0) THEN
           RB2(J,I) = RB2(I,J)
         ELSE
           IF(mynode.EQ.0)WRITE(13,*) 'RB2 not set for',I,J
         ENDIF
         RMAX(I,J) = RB2(I,J)
...
C Something to do with the expon
...
C
      ...
      REG(2,1,iflor)=EXP(XDB(2,1,iflor)*(RHF-RCH))
      REG(2,iflor,1)=EXP(XDB(2,iflor,1)*(RCH-RHF))
c
...
c$$$      AD(5,5)=471.18d0
c$$$      XTN2(5)=0.78734d0
c$$$      XTN1(5)=1/(2.0*XTN2(5))
c$$$      ADB(5)=1.0999D-6
c$$$      CDB(5)=1.0039D+5
c$$$      CDB2(5) = CDB(5)*cdb(5)
c$$$      DDB(5)=16.218d0
c$$$      DDB2(5) = DDB(5)*DDB(5)
c$$$      HDB(5)=0.59826d0
c$$$      RB1(5,5)=2.7d0
c$$$      RB2(5,5)=3.0d0
c$$$      RMAX(5,5)=RB2(5,5)
c$$$      PID(5,5)=PI/(RB2(5,5)-RB1(5,5))
c$$$      CHI(5,5) = 1.0d0
c$$$C
c$$$C TERSOFF GERMANIUM
c$$$C
c$$$      DXL(4,4) = 2.4451d0
c$$$      AXL(4,4) = 1.7047d0
c$$$      DD(4,4) = 1769.0d0
c$$$      AD(4,4) = 0.50d0 * 419.23d0
c$$$      XTN2(4) = 0.75627d0
c$$$      XTN1(4) = 1/(2.0*XTN2(4))
c$$$      ADB(4) = 9.0166d-07
c$$$      CDB(4) = 1.0643d+5
c$$$      CDB2(4) = CDB(4)*CDB(4)
c$$$      DDB(4) = 15.652D0
c$$$      DDB2(4) = DDB(4)*DDB(4)
c$$$      HDB(4) = -0.43884d0
c$$$      RB1(4,4)=2.7d0
c$$$      RB2(4,4)=3.0d0
c$$$      RMAX(4,4)=RB2(4,4)
c$$$      PID(4,4)=PI/(RB2(4,4)-RB1(4,4))
c$$$      CHI(4,4) = 1.0d0
c$$$C
c$$$C Mixed SILICON-GERMANIUM
c$$$C
c$$$      DXL(4,5) = (DXL(4,4)+DXL(5,5))/2.0d0
c$$$      DXL(5,4) = DXL(4,5)
c$$$      AXL(4,5) = (AXL(4,4)+AXL(5,5))/2.0d0
c$$$      AXL(5,4) = AXL(4,5)
c$$$      DD(4,5) = sqrt(DD(4,4)*DD(5,5))
c$$$      DD(5,4) = DD(4,5)
c$$$      AD(4,5) = sqrt(AD(5,5)*AD(4,4))
c$$$      AD(5,4) = AD(4,5)
c$$$      RB1(4,5) = sqrt(RB1(5,5)*RB1(4,4))
c$$$      RB1(5,4) = RB1(4,5)
c$$$      RB2(4,5) = sqrt(RB2(5,5)*RB2(4,4))
c$$$      RB2(5,4) = RB2(4,5)
c$$$      RMAX(4,5) = RB2(4,5)
c$$$      RMAX(5,4) = RMAX(4,5)
c$$$      PID(4,5) = PI/(RB2(4,5)-RB1(4,5))
c$$$      PID(5,4) = PID(4,5)
c$$$      CHI(4,5) = 1.00061d0
c$$$      CHI(5,4) = CHI(4,5)
c
...
C
      WRITE(13,*)'Check nieghbor list cutoff,rmax,rmin'
      DO 61 I=1,RTYPES
           DO 60 J=1,RTYPES
                RLIST(I,J)=(RMAX(I,J)+RLL)**2
                RMAX(I,J)=RMAX(I,J)**2
                RMIN(I,J)=RB1(I,J)**2
                WRITE(13,130) I,J,RLIST(I,J),RMAX(I,J),RMIN(I,J)
   60      CONTINUE
   61 CONTINUE
      RETURN
