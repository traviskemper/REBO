...
CCC 1-D parallel
      IF (NNDP.EQ.1) THEN
        ...
        node_cell_min_temp(NPD_i)=node_cell_min(NPD_i)+1
        node_cell_max_temp(NPD_i)=node_cell_min(NPD_i)+3
...
C                  ENDIF
        ...
        node_cell_min_temp=node_cell_min
        node_cell_max_temp=node_cell_max
        node_cell_max_temp(NPD_i)=node_cell_max(NPD_i)-1
        node_cell_min_temp(NPD_i)=node_cell_max(NPD_i)-3
...
C                  ENDIF
          ...
          ENDDO
        ENDDO
        N_atom_passLL(2)=NLL/15
        N_atom_passRR(2)=NRR/15
c        WRITE(*,*) 'find_nabor3,NLL,NRR',mynode,NLL/15,NRR/15
CCC 2-D parallel
      ELSE
        node_cell_min_temp=node_cell_min
        node_cell_max_temp=node_cell_max
...
C                  ENDIF
        ...
        node_cell_min_temp=node_cell_min
        node_cell_max_temp=node_cell_max
        node_cell_min_temp(NPD_j)=node_cell_min(NPD_j)+1
        node_cell_max_temp(NPD_j)=node_cell_min(NPD_j)+3
C Find the DOWN side atoms that will be passed to DOWN node
     ...
     &        *nncell(1)
              ...
              DO while (ii.NE.0)
                ...
                IF ((R_j.GT.2*RLIMAX).AND.(R_j.LE.3*RLIMAX)) THEN
...
C                  ENDIF
        ...
        node_cell_min_temp=node_cell_min
        node_cell_max_temp=node_cell_max
        node_cell_min_temp(NPD_j)=node_cell_max(NPD_j)-3
        node_cell_max_temp(NPD_j)=node_cell_max(NPD_j)-1
...
C Find the LEFT DOWN side atoms that will be passed to LEFT DOWN side
     ...
     &        *nncell(1)
              ...
              DO while (ii.NE.0)
                R_i=ABS(R0_node(NPD_i,ii)-pmin_node(NPD_i))
                R_j=ABS(R0_node(NPD_j,ii)-pmin_node(NPD_j))
                IF (((R_i.GT.2*RLIMAX).AND.(R_i.LE.3*RLIMAX).AND.
     &             (R_j.LE.3*RLIMAX)).OR.((R_j.GT.2*RLIMAX).AND.
     &             (R_j.LE.3*RLIMAX).AND.(R_i.LE.3*RLIMAX))) THEN
...
C                  ENDIF
        ...
        N_atom_passLL(2)=NLL/15
        N_atom_passRR(2)=NRR/15
        N_atom_passLD(2)=NLD/15
        N_atom_passRU(2)=NRU/15
        N_atom_passRD(2)=NRD/15
        N_atom_passLU(2)=NLU/15
        N_atom_passDD(2)=NDD/15
        N_atom_passUU(2)=NUU/15
C        WRITE(*,*)'find_nabor3',mynode,NLL/15,NRR/15,NDD/15,NUU/15,'LL'
C        WRITE(*,*)'find_nabor3',mynode,NLD/15,NLU/15,NRD/15,NRU/15,'LD'
      ENDIF
